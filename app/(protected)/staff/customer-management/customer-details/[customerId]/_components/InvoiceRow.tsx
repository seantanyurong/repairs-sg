...
export default function InvoiceRow({
  ...
  const handleVoidInvoice = async () => {
    ...
    try {
      ...
      toast('Invoice Void Successfully', {
        ...
        action: {
          label: 'View Invoice',
          onClick: () => router.push(`/staff/invoices/view-invoice/${invoiceId}`),
        },
      });
    } catch (error) {
  ...
  return (
    <>
      <TableRow>
        <TableCell className={isVoid ? 'opacity-50 cursor-not-allowed' : ''}>{invoiceId.toString()}</TableCell>
        <TableCell className={isVoid ? 'opacity-50 cursor-not-allowed' : ''}>{customer}</TableCell>
        <TableCell className={isVoid ? 'opacity-50 cursor-not-allowed' : ''}>{job}</TableCell>
        <TableCell className={isVoid ? 'opacity-50 cursor-not-allowed' : ''}>${totalAmount.toString()}</TableCell>
        <TableCell className={isVoid ? 'opacity-50 cursor-not-allowed' : ''}>${remainingDue.toString()}</TableCell>
        ...
        <TableCell className={isVoid ? 'opacity-50 cursor-not-allowed' : ''}>{paymentStatus}</TableCell>
        <TableCell>
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            <DropdownMenu modal={false}>
              ...
              <DropdownMenuContent align='end'>
                ...
                <DropdownMenuItem disabled={isVoid || isPaid} onClick={handleVoidAction} className='cursor-pointer'>
                  Void
                </DropdownMenuItem>
                <DropdownMenuItem disabled={isVoid || isPaid} onClick={handlePaymentAction} className='cursor-pointer'>
                  Payment
                </DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
            <DialogContent>
              <DialogHeader>
                <DialogTitle>{actionType === 'void' ? 'Are you absolutely sure?' : 'Process Payment'}</DialogTitle>
                <DialogDescription>
                  {actionType === 'void' ? (
